...
def lambda_handler(event, context):
    ...
    html_res = get_results_page()
    
    # detected = result_detection(expected_sonnendach_api,year_user_input_data)
    # $VALUE_LIST$
    # [{x: 1, y: 12.0}, {x: 2.0, y: 33.1}, {x: 3.00, y: 6.33}, {x: 4.00, y: -0.0}]
    
    results_str = get_results_page()
    
    new_test = results_str.replace("$LAT$", str(coord[2]))
    newer_test = new_test.replace("$LONG$", str(coord[3]))
    
    print(newer_test)
    
    print(pv_has_malfunction_with_correction(float(area), float(kwp), expected_sonnendach_api, year_user_input_data))
    out = {}
    out['statusCode'] = 200
    out['body'] = '{"1": 1}'
